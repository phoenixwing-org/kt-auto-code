...
- 用户于 2026-09-11 确认补丁本地安装测试通过，并反馈已发布到插件市场；本次归档源码与验收记录，不重复发布或覆盖已验收包。

- 修复 0.9.2 Primary 首次加载为空：Run 入口独立注册其 Wing 清理弹窗，不再依赖已迁入 Right 的编译工具入口；避免 `runCleanupDialog.close is not a function` 中断目录、工具栏和欢迎页渲染。
- 修复编译工具 Right「清理」持续置灰：兼容 VS Code 将 `window.parent` 隐藏为自身的隔离机制，按 Webview 同源转发接收 Host 状态；拒绝外部来源并保留所有清理令牌与执行门禁。
- 编译工具「清理」入口始终可点击；未就绪、没有可用目录或任务忙时在弹窗中说明原因，预览与执行仍受保护，关闭提示不会取消正在运行的构建。
- 清理以当前工作目录为默认范围：配置目录为空时使用传入目录，规则/Git/CMake 模式切换只默认选择当前目录；ROOT、3rdParty 和其他项目需手动勾选。YAML 探测使用同一目录且不向外扩展；目录改变废弃旧来源和预览。
- 清理 Header 显示“目录名 @ 父路径”，长路径省略并保留完整提示；“在 VS Code 中编辑”移至清理规则标题行，不触发折叠。Git 仅接受当前目录本身为仓库根目录，CMake 仅清空其 `build` 内容。
- 修复长目录撑宽清理对话框并裁掉右侧按钮：约束内部 Grid 列宽、截断标题和目标路径；窄窗口 Header 动作可换行，关闭与规则编辑按钮保留在边界内。无目录提示和 Git 确认保持不变。
- 新增生产脚本真实 DOM 初始化回归及 VSIX 包内门禁，覆盖空窗口、有工作区、工具栏显示和 Run 激活，并验证缺少注册的旧故障会被拒绝。Wing 精确依赖与生成规则 `1.0.2` 不变。

## 0.9.2（已发布，发现 Primary 初始化缺陷）
...
- Wing 修正构造函数 `clang-format on` 与 END 的缩进，跟随后续初始化项或独立 `{`；独立生成规则升级为 `1.0.1`，旧预检缓存失效。两组夹具覆盖 LF/CRLF 与重复生成，macOS 格式化通过；Windows 未验，不批量改写用户工程。
- PnwCombo 保持 Wing 唯一运行时和原事件，Auto 删除重复契约及强制转换；修复焦点返回、删除后模型刷新、键盘出口、禁删原因无障碍名称和 Shadow Host 裁剪边界，保留 200 项上限。
- 本地类型检查使用受控 Wing dist 声明并拒绝 Registry 回退，不向 manifest、lockfile 或 tsconfig 写入本机路径。验证与归档边界见[开发计划](docs/0.9.2-开发计划.md)。
- 联动 Wing 0.7.4 本地候选；0.9.0 不正式发布。原冻结批次不含清理对话框和编译工具后续，新授权开发另记在后续计划，不回改冻结制品。
- 后续授权接入真实 `cleanup.yaml` 探测、VS Code 原生编辑及逐配置清理；仅从当前工作目录向下发现，跳过/限额明细进入 Output，列表保留摘要。清理入口与弹窗临时移至编译工具 Right，顶部动作、四个折叠 Block 与固定顶边共用 Wing 组件；独立浮窗仍待隔离验证。
- 编译工具原型补齐项目表、清单导入、配置/脚本窗口及任务状态闭环；仍用内存模拟，不冒充正式 clone、导出或构建。Windows 导出脚本首行错误因缺少原始报错保留待定位。
- Combo 参数回填补齐当前字段注释，生成规则与缓存同步升级到 `1.0.2`；原生下拉选项补齐前景/背景主题配对。Windows 多主题与 CAA 运行验收单列，不能以 macOS 单测替代。

## 0.9.1（本地内测，尚未正式发布）

- C++ 成员排序、头文件 ASCII 修正、编码修正、UUID 替换及 CAA UI 统一采用首行紧凑文字工具栏；五项 Preview 已确认并接入正式 Host，保留原有扫描、确认、定位、差异和外部工具能力，不新增 Right 或改变外层 Shell。
- 正式 Primary 使用共享组件和独立模型适配，保留每工具状态；UUID 生成策略与冻结扫描计划不一致时禁止旧计划写入，迟到状态不能覆盖新选择。
- Git 合并区间仅显示当前 HEAD 涉及的历史与分叉，兼顾时间顺序和父子拓扑；非当前主线提交置灰禁选。Right 提供当前分支切换，确认后清空旧选择；未提交改动、其他 worktree 占用及旧会话漂移均明确阻止并提示。
- Git 长提交说明自动截断，保留右侧 hash、作者和时间；新增 `remote` / `remote ×N` 本地远端跟踪引用标记。remote 使用问号光标、点击展开详情，长文本延迟 500ms 提示，普通分支标签无专属 hover 效果。
- 自动代码 JSON Right 修正内容高度传递，恢复内部垂直滚动；预检详情按内容区而非整个窗口计算可用高度，随工具栏换行和视图尺寸更新。重新激活 JSON Right 时恢复对应自动代码 Primary，保留各 JSON 草稿，不启动扫描或抢走编辑焦点。
- 自动代码 Header 整合为“自动代码 · JSON 文件名 @ 目录”和右侧文档操作；预检结果只保留一个可折叠 Block Header，合并筛选、路径显示和计划状态，保留列表与详情滚动。
- 参数表与预检结果统一 Block Header 字体、间距、箭头和折叠点击范围；点击标题或空白处也可折叠，右侧工具与筛选独立操作。JSON 文档标签改名留到 0.9.2，本次不改生成规则。
- 生成注释采用独立 `@codegen-rules-version 1.0.0`，不再沿用旧 App 版本与年份；预检缓存同步规则版本并隔离运行时来源，防止复用旧生成结果。旧源码不批量改写，构造函数结束标记的格式化问题仅记录样例、暂不修复。
- 本地内测包嵌入已验证的并列 Wing，不升级 Registry 依赖，不覆盖 0.9.0 归档。正式发布仍须通过 Registry 能力与发布来源门禁。
- Git“合并区间”Right 原型补齐按用户决定移至 0.9.2；清理快捷栏和 Phoenix 缓存清理仍为后续 TODO。本版保持 `.phoenix/cache`，不迁移或删除 clangd 缓存。
...
- 0.9.0 不正式发布；已有本地内测归档保留，后续改进进入开发分支。
...
- Ignore View 的添加操作会检查目标草稿，已有规则不重复追加；新增当前 Git/Phoenix 目标的“修正（N）”与显式保存，修正目前清理重复规则并保留首条、注释、顺序和目录/大小写语义差异。
- 合入 0.8.3 的统一 Ignore、项目改名安全边界与 Primary Toolbar Strip；0.8.3 仍保留独立归档和本地标签。
