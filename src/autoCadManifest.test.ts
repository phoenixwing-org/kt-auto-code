...
describe("KT Auto CAD extension manifest", () => {
  ...
  it("keeps product-owned IDs separate and leaves the Activity Bar to the base extension", () => {
    ...
    expect(cad.devDependencies?.["@phoenix-wing/cad-core"]).toBe("0.5.1");
    expect(cad.devDependencies?.["@phoenix-wing/cad-contracts"]).toBe("0.5.1");
    expect(cad.devDependencies?.["@phoenix-wing/workspace-schema"]).toBe("0.5.1");
    ...
    expect(code.dependencies?.["@phoenix-wing/code-core"]).toBe("0.5.1");
    expect(code.dependencies?.["@phoenix-wing/git-core"]).toBe("0.5.1");
    expect(code.dependencies?.["@phoenix-wing/git-node"]).toBe("0.5.1");
    expect(code.dependencies?.["@phoenix-wing/kt-codegen"]).toBe("0.5.1");
    expect(code.dependencies?.["@phoenix-wing/run-core"]).toBe("0.5.1");
    expect(code.dependencies?.["@phoenix-wing/run-node"]).toBe("0.5.1");
