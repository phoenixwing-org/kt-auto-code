...
describe("CAA Desk Tools bridge", () => {
  it("derives the health endpoint from a configured open endpoint", () => {
    expect(ktcCaaHealthEndpoint("http://127.0.0.1:48375/api/caa/dialog/open?x=1"))
      .toBe("http://127.0.0.1:48375/api/caa/health");
  });

  it("distinguishes online, offline and incompatible services", async () => {
    await expect(ktcProbeDeskTools("http://127.0.0.1:48375/api/caa/dialog/open", async () =>
      new Response(JSON.stringify({ ok: true, service: "caa", protocol_version: 1 })))).resolves.toMatchObject({ status: "online", text: "Desk Tools 桌面服务已连接 · 127.0.0.1:48375" });
    await expect(ktcProbeDeskTools("http://127.0.0.1:48375/api/caa/dialog/open", async () =>
  ...
  it("submits the workspace and file and reports server rejection", async () => {
    ...
    await ktcSubmitCaaDialog("http://127.0.0.1:48375/api/caa/dialog/open", { workspaceRoot: "/work", file: "/work/A.CATDlg" }, fetcher);
    expect(fetcher).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ method: "POST" }));
    await expect(ktcSubmitCaaDialog("http://127.0.0.1:48375/api/caa/dialog/open", { file: "/work/A.CATDlg" }, async () =>
