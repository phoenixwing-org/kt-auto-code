...
const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "kt-auto-cad-launch-test-"));
const cadRoot = path.join(fixtureRoot, "kt-auto-cad");
fs.mkdirSync(cadRoot);
fs.writeFileSync(path.join(cadRoot, "package.json"), JSON.stringify({ name: "kt-auto-cad" }));
...
describe("Extension Development Host launcher", () => {
  ...
  it("keeps an explicit Code-only mode", () => {
    ...
    expect(result.stdout).not.toContain(cadRoot);
  });
  ...
  it("uses an isolated snapshot and a new window for local Wing development", () => {
    ...
    expect(result.stdout.match(/--extensionDevelopmentPath=/g)).toHaveLength(2);
    expect(result.stdout).not.toContain(`--extensionDevelopmentPath=${root}`);
  });
...
function runDryLaunchWithEnvironment(
  ...
  return spawnSync(process.execPath, [launchScript, ...args, `--cad-root=${cadRoot}`, "--dry-run"], {
    cwd: root,
    encoding: "utf8",
    env: {
      ...process.env,
