...
describe("workspaceRename", () => {
  ...
  it("只读搜索允许替换内容为空并覆盖文本、文件名和文件夹名", () => {
    const root = tempRoot();
    mkdirSync(join(root, "OldFolder"));
    const file = join(root, "OldFolder", "OldFile.txt");
    ...
    expect(report.summary).toMatchObject({ directories: 1, files: 1, textFiles: 1, replacements: 3, errors: 0 });
    expect(reportLevels(report)).toEqual(["text", "file", "dir"]);
    expect(readFileSync(file, "utf8")).toBe("Old value\n");
    expect(readdirSync(root)).toEqual(["OldFolder"]);
    expect(report.hits.every((hit) => hit.originalFullPath === hit.plannedFullPath)).toBe(true);
  });
