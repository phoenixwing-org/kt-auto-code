...
describe("sidebar panel HTML", () => {
  ...
  it("使用统一关联规则对话框而不是多套 Quick Pick 消息", () => {
    ...
    expect(html).toContain('id="btn-toggle-working-context"');
    expect(html).toContain('id="btn-toggle-primary-block"');
    expect(html.match(/<svg class="shell-block-chevron" viewBox="0 0 16 16" aria-hidden="true">/gu)).toHaveLength(3);
    ...
    expect(html).toContain('state.workingContextCollapsed = !state.workingContextCollapsed');
    ...
    expect(html).toContain('#primary-body { min-height: 0; flex: 1 1 auto; overflow-x: hidden; overflow-y: auto; }');
