...
describe("Git Primary panel", () => {
  it("提供简报编辑复制、分支切换和提交图合并入口", () => {
    ...
    expect(source).toContain("const primaryCommits = project.commits.slice(0, 2)");
    expect(source).toContain("for (const commit of primaryCommits)");
    expect(source).toContain('historyTitle.textContent = `更多 commit（已加载 ${Math.max(0, project.commits.length - 2)}）`');
    expect(source).toContain("for (const commit of project.commits.slice(2))");
    ...
    expect(source).toContain("projects.append(this.KtcGitActionTree(selectedProject, model))");
    expect(source).toContain("const content: Node[] = []");
    ...
    expect(source).not.toContain('title.textContent = "当前工作区未发现 Git 仓库"');
    expect(source).not.toContain('"可以在工作区根目录新建仓库，或递归搜索子目录中的现有仓库。"');
    expect(source).not.toContain('["新建 Git 仓库", "initializeRepository"]');
    expect(source).toContain('label: searching ? "停止搜索所有子目录" : "搜索所有子目录"');
    expect(source).toContain('this.KtcEmit({ action: "searchRepositories" })');
  ...
  it("由 VS Code Host 发现并保存当前仓库选择", () => {
    ...
    expect(controller).toContain("this.KtcSummaryDraft?.repositoryId !== this.KtcSelectedRepositoryId");
    expect(controller).toContain("selectedRepositoryInput.loaded === false");
    expect(controller).toContain("this.KtcAdapter.readRepositorySummary(directory.root, 2, true, cancellation.signal)");
    expect(controller).toContain("const loadedCommitCount = this.KtcSessions.get(selected)?.snapshot.history.length ?? 0");
    expect(controller).toContain("selected=${selected} commits=${loadedCommitCount}");
    expect(controller).not.toContain("selected=${selected} commits=1");
    ...
    expect(controller).not.toContain('refsScope: KtcPnwGitCommitGraphRefsScope = "local-branches"');
    expect(controller).toContain('refsScope: "head"');
    ...
    expect(controller).toContain("KtcProjectGitRangeSelection(commits, initialSelection, firstParentOids)");
    expect(controller).toContain("KtcLoadedGitFirstParentOids(commits, page.headOid)");
    expect(controller).toContain("graph.firstParentOids");
    expect(controller).toContain("所选 commit 不属于当前分支，不能合并");
    ...
    expect(controller).toContain("带入的 ${projection.missingOids.length} 个 commit 不在当前分支提交图中");
    ...
    expect(controller).toContain('const prefix = message.type === "selectBranch" ? "分支切换未完成：" : "合并未执行："');
    expect(controller).toContain('this.KtcShowSquashView(repositoryId, "error", prefix + KtcErrorMessage(error), this.KtcSquashDraft)');
    expect(controller).toContain('readyMessage ?? "已读取最近 5 条当前分支提交图；按需继续加载。"');
    ...
    expect(controller).toContain("KtcReadLocalGitBranchOptions(page.root)");
    expect(controller).toContain('message.type === "selectBranch"');
    expect(controller).toContain("KtcSelectBranchFromSquashView(repositoryId, message.branchName, ctx)");
    ...
    expect(controller).toContain("KtcAssessGitBranchRange(branches, currentBranchName, trustedSelectedOids)");
