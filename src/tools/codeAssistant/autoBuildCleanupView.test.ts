...
afterEach(() => { disposers.splice(0).forEach((dispose) => dispose()); vi.restoreAllMocks(); document.body.replaceChildren(); });
...
function cleanup(patch: Partial<KtcCleanupDialogModel> = {}): KtcCleanupDialogModel {
  ...
  targets: [{ id: "work", label: "工作目录", path: "/safe/work", selected: true, supportedModeIds: ["rules"] },
    { id: "repo", label: "仓库", path: "/safe/repo", selected: false, supportedModeIds: ["git-force"] }],
...
function setup() {
  ...
  const send = (patch: { contextId?: string; revision?: number; ready?: boolean; enabled?: boolean; busy?: boolean;
    ...
    const snapshot: KtcEditorPrimaryCompanionSnapshot = {
      ...
      actions: [{ id: "cleanupDialog", label: "清理", enabled: patch.enabled ?? true }],
    ...
    window.dispatchEvent(new MessageEvent("message", { data: { type: "autoBuildCleanupState", snapshot } }));
    return snapshot;
  };
...
describe("AutoBuild Right cleanup adapter", () => {
  it("首个snapshot只启用Right按钮；点击才打开并探测一次，继续复用Wing和YAML组件", () => {
    const view = setup();
    expect(view.button.disabled).toBe(true); expect(view.dialog.open).toBe(false);
    view.send({ ready: false }); expect(view.button.disabled).toBe(true);
    ...
    expect(Array.from(view.dialog.querySelectorAll('[slot="header-actions"]')).map((item) => item.textContent)).toEqual(["在 VS Code 中编辑", "探测配置"]);
  });
  ...
  it("同context更新保持本地mode/目标，当前预检token才能执行；忙时禁用其他动作但可取消", () => {
    const view = setup(); view.open();
    const changed = cleanup({ selectedModeId: "git-force", targets: cleanup().targets.map((target) => ({ ...target, selected: target.id === "repo" })) });
    ...
    expect(view.dialog.model.targets.filter(({ selected }) => selected).map(({ id }) => id)).toEqual(["repo"]);
    view.action({ kind: "preview" });
    expect(view.payloads().at(-1)).toEqual({ kind: "preview", request: { modeId: "git-force", targetIds: ["repo"], rulesYaml: changed.rulesYaml } });
  ...
  it("旧revision被忽略，context变化关闭但不自动重开，旧context迟到也不能认领", () => {
    ...
    expect(view.dialog.open).toBe(false); expect(view.button.disabled).toBe(true);
  });
