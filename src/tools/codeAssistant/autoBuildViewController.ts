...
export class KtcAutoBuildViewController implements vscode.Disposable {
  ...
  private async handle(message: Message): Promise<void> {
    if (message.type === "ready") {
      ...
      const configuration = saved ? { ...inferred, ...saved, rootDirectory: saved.rootDirectory?.trim() || inferred.rootDirectory, thirdPartyDirectory: saved.thirdPartyDirectory?.trim() || inferred.thirdPartyDirectory, workingDirectory: this.defaultWorkingDirectory || saved.workingDirectory?.trim() || inferred.workingDirectory } : inferred;
      await this.post({ type: "configuration", configuration, detectedRootDirectory: this.detectedRootDirectory, path: "", recentPaths: this.workspaceState.get<string[]>(RECENT_KEY) || [], platform: process.platform }); await this.postScriptStatus(); return;
    }
  ...
  private async postScriptStatus(): Promise<void> { const source = vscode.Uri.joinPath(this.extensionUri, "scripts", "auto-build", "Invoke-AutoBuild.ps1").fsPath, target = this.detectedRootDirectory ? ktcJoinAutoBuildPath(this.detectedRootDirectory, "tools", "Invoke-AutoBuild.ps1") : ""; if (target && !ktcCanAccessAutoBuildPathOnHost(target, process.platform)) { await this.post({ type: "scriptStatus", status: "foreign", source, target, sourceHash: "", targetHash: "" }); return; } let status = "missing", sourceHash = "", targetHash = ""; try { sourceHash = createHash("sha256").update(await readFile(source)).digest("hex"); if (target) targetHash = createHash("sha256").update(await readFile(target)).digest("hex"); status = sourceHash === targetHash ? "same" : "different"; } catch { status = target ? "missing" : "unavailable"; } await this.post({ type: "scriptStatus", status, source, target, sourceHash, targetHash }); }
  ...
  private async runProcess(task: KtcAutoBuildTask, args: string[], configuration: KtcAutoBuildConfiguration): Promise<number> { const projectName = task.path?.replace(/[\\/]+$/, "").split(/[\\/]/).at(-1), phaseName = task.phase === "cmake" ? "CMake" : task.phase === "caa" ? "CAA" : task.phase === "export" ? "Export" : task.phase === "link" ? "LinkCAA" : "Repositories", logTag = projectName ? `${phaseName}-${projectName}` : phaseName; if (process.platform === "win32" && [configuration.rootDirectory, configuration.thirdPartyDirectory, task.path].filter((path): path is string => !!path).some((path) => !ktcCanAccessAutoBuildPathOnHost(path, process.platform))) throw new Error("Windows 实际执行仅接受盘符绝对路径或 UNC 共享根路径。"); this.warnNonWindowsBlindRun(); this.log(`任务 ${task.name}；命令 powershell.exe ${task.commandSummary}`, true); return await new Promise<number>((resolve) => { const child = spawn("powershell.exe", args, { windowsHide: true, env: { ...process.env, ROOT_DIR: configuration.rootDirectory, ROOT_DIR_3rdParty: configuration.thirdPartyDirectory } }); this.processes.add(child);
    const streams = { stdout: { decoder: new StringDecoder("utf8"), pending: "" }, stderr: { decoder: new StringDecoder("utf8"), pending: "" } };
    const write = (kind: "stdout" | "stderr", value: Buffer) => { const stream = streams[kind]; stream.pending += stream.decoder.write(value); const lines = stream.pending.split(/\r?\n/); stream.pending = lines.pop() || ""; for (const line of lines) this.output.appendLine(line ? `[Auto Build][${logTag}][${kind}] ${line}` : ""); };
