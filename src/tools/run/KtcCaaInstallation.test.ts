...
describe("CAA machine installation resolution", () => {
  it("uses the documented C:\\DS convention and Windows 64 runtime by default", () => {
    const installation = KtcResolveCaaInstallation({ version: "R20" });
    expect(installation).toEqual({
      radeRoot: "C:\\DS\\RADE20",
      catiaRoot: "C:\\DS\\B20",
      runtimeDirectory: "win_b64",
    });
    expect(KtcCaaRadeCommandRoot(installation)).toBe("C:\\DS\\RADE20\\win_b64");
  });
  ...
  it("rejects invalid machine values before they reach cmd.exe", () => {
    expect(() => KtcResolveCaaInstallation({ version: "20", radeRoot: "C:/DS/& unsafe" }))
      .toThrow("CAA 安装目录无效");
    expect(() => KtcResolveCaaInstallation({ version: "20", runtimeDirectory: "arm64" }))
      .toThrow("CAA 平台目录无效");
  });
  ...
  it("keeps the bundled cmd protocol aligned with the resolved machine values", () => {
    ...
    expect(runner).toContain('set "PNW_BASE=%PNW_RADE_ROOT%\\%PNW_RUNTIME%"');
