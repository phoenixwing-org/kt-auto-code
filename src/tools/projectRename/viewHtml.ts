...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  body { margin: 0; padding: 0; overflow: hidden; color: var(--vscode-foreground); background: var(--vscode-editor-background); font: 13px/1.45 var(--vscode-font-family); }
  ...
  main { max-width: 1240px; margin: 0 auto; padding: 8px; }
  h1,h2 { margin: 0; } h1 { font-size: 20px; } h2 { font-size: 14px; }
  button,input,select { font: inherit; } button { min-height: 28px; padding: 3px 10px; color: var(--vscode-button-secondaryForeground); background: var(--vscode-button-secondaryBackground); border: 1px solid var(--vscode-button-border, var(--vscode-contrastBorder, transparent)); border-radius: 2px; cursor: pointer; }
  ...
  button:focus-visible,input:focus-visible,select:focus-visible,.results:focus-visible { outline: 2px solid var(--pnw-focus-ring, var(--vscode-contrastActiveBorder, var(--vscode-focusBorder))); outline-offset: 1px; }
  input[type=text] { width: 100%; min-height: 28px; padding: 4px 7px; color: var(--vscode-input-foreground); background: var(--vscode-input-background); border: 1px solid var(--vscode-input-border, var(--vscode-contrastBorder, var(--vscode-panel-border, transparent))); }
  input[type=text]:disabled, select:disabled { color: var(--vscode-disabledForeground, var(--vscode-descriptionForeground)); opacity: .72; }
  select { width: 100%; min-height: 28px; padding: 3px 7px; color: var(--vscode-dropdown-foreground); background: var(--vscode-dropdown-background); border: 1px solid var(--vscode-dropdown-border, var(--vscode-contrastBorder, var(--vscode-panel-border, transparent))); }
  input[type=checkbox] { width: 16px; height: 16px; }
  .muted { color: var(--vscode-descriptionForeground); }
  .header-actions { display: flex; width: 100%; min-width: 0; align-items: center; justify-content: flex-end; gap: 7px; overflow-x: auto; scrollbar-width: none; }
  .header-actions::-webkit-scrollbar { display: none; }
  .header-actions button { flex: none; white-space: nowrap; }
  .analysis-context { display: grid; min-height: 34px; grid-template-columns: auto minmax(0,1fr); align-items: center; gap: 8px; padding: 5px 9px; border: 1px solid var(--pnw-workbench-border, var(--vscode-contrastBorder, var(--vscode-panel-border, transparent))); background: var(--vscode-sideBarSectionHeader-background); }
  .analysis-context strong { font-size: 12px; }
  .analysis-context span { min-width: 0; overflow: hidden; color: var(--vscode-descriptionForeground); text-overflow: ellipsis; white-space: nowrap; }
  ...
  .section { margin-top: 12px; overflow: hidden; border: 1px solid var(--pnw-workbench-border, var(--vscode-contrastBorder, var(--vscode-panel-border, transparent))); background: var(--pnw-workbench-surface, var(--vscode-editor-background)); }
  ...
  details.section[open] > summary { border-bottom: 1px solid var(--pnw-workbench-border, var(--vscode-contrastBorder, var(--vscode-panel-border, transparent))); }
  ...
  .scheme-table { min-width: 700px; border-top: 1px solid var(--pnw-workbench-border, var(--vscode-contrastBorder, var(--vscode-panel-border, transparent))); }
  .scheme-table-row,.rule { display: grid; min-height: 36px; grid-template-columns: 48px 124px minmax(180px,1fr) minmax(180px,1fr) 42px; align-items: center; gap: 8px; padding: 3px 7px; border-bottom: 1px solid var(--pnw-workbench-border, var(--vscode-contrastBorder, var(--vscode-panel-border, transparent))); }
  .scheme-table-head { min-height: 30px; color: var(--vscode-descriptionForeground); background: var(--vscode-sideBarSectionHeader-background); font-size: 11px; }
  .scheme-table-head span:last-child { text-align: center; }
  .scheme-table-row > input[type=checkbox],.rule > input[type=checkbox] { justify-self: center; }
  .scheme-table-row > input[type=text],.rule > input[type=text] { min-width: 0; }
  .profile-panel { margin: 8px 0; border-top: 1px solid var(--vscode-contrastBorder, var(--vscode-panel-border, transparent)); border-bottom: 1px solid var(--vscode-contrastBorder, var(--vscode-panel-border, transparent)); }
  ...
  .profile-fields { display: grid; grid-template-columns: minmax(210px,.8fr) minmax(300px,1.2fr); gap: 8px; padding: 7px 5px 8px; border-top: 1px solid var(--vscode-contrastBorder, var(--vscode-panel-border, transparent)); }
  ...
  .related-candidates { margin-top: 10px; padding: 8px; border: 1px solid var(--pnw-workbench-border, var(--vscode-contrastBorder, var(--vscode-panel-border, transparent))); background: var(--vscode-textBlockQuote-background); }
  ...
  .metric { min-width: 0; padding: 8px; background: var(--vscode-editorWidget-background); border: 1px solid var(--vscode-contrastBorder, var(--vscode-panel-border, transparent)); }
  ...
  .completion { display: grid; grid-template-columns: auto repeat(3,auto) minmax(0,1fr); align-items: center; gap: 7px; margin-top: 8px; padding: 7px; border: 1px solid var(--vscode-contrastBorder, var(--vscode-panel-border, transparent)); }
  .completion .gate { padding: 1px 6px; border: 1px solid var(--vscode-contrastBorder, var(--vscode-panel-border, transparent)); border-radius: 999px; white-space: nowrap; }
  .completion .passed { color: var(--vscode-testing-iconPassed); }
  .results { max-height: min(60vh, 720px); overflow: auto; }
  table { width: 100%; min-width: 920px; border-collapse: separate; border-spacing: 0; table-layout: fixed; }
  th,td { height: 34px; padding: 4px 7px; overflow: hidden; text-align: left; vertical-align: middle; border-bottom: 1px solid var(--vscode-contrastBorder, var(--vscode-panel-border, transparent)); text-overflow: ellipsis; white-space: nowrap; }
  ...
  .col-action { position: sticky; z-index: 2; right: 0; width: 116px; background: var(--vscode-editor-background); box-shadow: -1px 0 var(--vscode-contrastBorder, var(--vscode-panel-border, transparent)); }
  ...
  @media (max-width: 760px) { .summary { grid-template-columns: 1fr 1fr; } .completion { grid-template-columns: 1fr 1fr; } .completion strong,.completion .message { grid-column: 1 / -1; } .profile-panel > summary { flex-wrap: wrap; } .profile-actions { margin-left: 18px; } .related-candidate { grid-template-columns: minmax(100px,1fr) 20px minmax(100px,1fr); } .related-candidate .count,.related-candidate button { grid-column: 3; justify-self: start; } }
</style></head><body>
<ktc-right-view-shell id="right-shell">
  <div class="header-actions" slot="actions"><button id="choose-root" hidden>选择目录…</button><button id="analyze" class="primary">分析</button><button id="cancel" hidden>取消</button><button id="preview-diff" disabled>预览差异…</button><button id="apply" disabled>执行改名</button><button id="git-changes" hidden disabled>Git 对比</button><button id="finish" disabled>结束任务</button></div>
<main>
  <div class="analysis-context" hidden><strong>分析目录</strong><span id="root">未选择分析目录</span></div>
  <div id="notice" class="notice quiet" hidden>正在准备分析器…</div>
  <details class="section" open><summary class="section-title"><h2>改名方案</h2><span id="rules-count" class="count">0 条启用</span><span class="scope-note" title="当前固定处理文本、文件名和文件夹名；ASCII 原文件写入非 ASCII 目标时默认使用 UTF-8">默认：文本 · 文件名 · 文件夹名 · UTF-8</span><span class="header-history" data-section-action hidden><select id="rename-history" aria-label="最近输入和项目方案" title="恢复最近输入或当前目录的项目方案"><option value="">最近输入 / 项目方案…</option></select><button id="delete-history" class="icon" type="button" title="删除所选本机记录" aria-label="删除所选最近记录" disabled>×</button><button id="clear-history" type="button" title="清空用户最近输入和所有项目的本机方案" aria-label="清空全部本机改名历史" disabled>清空</button></span><span class="section-title-actions"><button id="add-rule" data-section-action title="添加一条自定义规则" aria-label="添加自定义规则">+ 规则</button><button id="common-rules" data-section-action title="选择常用关联规则" aria-label="选择常用规则">常用</button><button id="caa-rules" data-section-action title="选择 CAA / C++ 关联规则" aria-label="选择 CAA 规则">CAA</button><button id="toggle-rules" data-section-action title="取消勾选全部规则" aria-label="取消勾选全部规则">全不选</button><button id="derive" data-section-action title="根据项目名和前缀重新派生名称形态" aria-label="重新派生名称形态">派生</button></span></summary><div class="body">
    <div class="scheme-table-scroll" role="region" aria-label="改名方案表格" tabindex="0"><div class="scheme-table" role="table">
      <div class="scheme-table-row scheme-table-head" role="row"><span role="columnheader">启用</span><span role="columnheader">类型</span><span role="columnheader">原来</span><span role="columnheader">目标</span><span role="columnheader">操作</span></div>
      <div class="scheme-table-row" role="row"><span aria-hidden="true"></span><span>项目名</span><input id="source-name" type="text" maxlength="256" aria-label="原项目名" /><input id="target-name" type="text" maxlength="256" aria-label="目标项目名" /><span aria-hidden="true"></span></div>
      <div class="scheme-table-row" role="row"><span aria-hidden="true"></span><span title="Web、CAA、C++ 等项目共用的显式源前缀；不自动猜测">前缀</span><input id="source-prefix" type="text" maxlength="256" placeholder="例如 Pnx / KTC" aria-label="原前缀（可选）" /><input id="target-prefix" type="text" maxlength="256" placeholder="例如 Phx / KTM" aria-label="目标前缀（可选）" /><span aria-hidden="true"></span></div>
      <div id="rules" class="rules" role="rowgroup"></div>
    </div></div>
    <details id="profile-panel" class="profile-panel" hidden><summary>项目档案<span id="profile-count" class="count">0 个共享档案</span></summary><div class="profile-fields"><label>项目规则档案<select id="profile"><option value="">项目规则档案…</option></select></label><label>项目档案名称<span class="profile-save"><input id="profile-name" type="text" maxlength="256" placeholder="例如：Phoenix 产品改名" title="明确保存后写入当前项目 .phoenix/search-replace.json" /><button id="save-profile" type="button" title="保存到当前项目 .phoenix/search-replace.json">保存</button></span></label></div><p id="profile-error" class="profile-error" hidden></p></details>
  ...
  <details id="summary-section" class="section" open hidden><summary class="section-title"><h2>完成门禁</h2><span class="count">写盘验收</span></summary><div class="body"><div id="summary" class="summary" hidden></div><div id="root-suggestion" class="root-suggestion" hidden><span id="root-suggestion-text"></span><button id="rename-root" type="button" hidden>重命名根目录…</button><span id="root-rename-reason" class="reason muted"></span></div><div id="completion" class="completion" hidden><strong>完成门禁</strong><span id="completion-target" class="gate">目标未验证</span><span id="completion-plan" class="gate">计划未执行</span><span id="completion-remaining" class="gate">剩余 0 项</span><span id="completion-message" class="message muted"></span></div></div></details>
  <details id="results-section" class="section" open hidden><summary class="section-title"><h2>命中与风险</h2><span id="result-count" class="count"></span></summary><div class="results" role="region" aria-label="项目改名命中结果" tabindex="0"><table><thead><tr><th class="col-risk">风险</th><th class="col-kind">分类</th><th class="col-source">来源</th><th class="col-target">目标 / 位置预览</th><th class="col-count">命中</th><th class="col-action">操作</th></tr></thead><tbody id="results"></tbody></table></div><div class="footer"><button id="load-more" hidden>加载更多</button></div></details>
</main></ktc-right-view-shell><ktc-associated-rule-picker id="rule-picker"></ktc-associated-rule-picker><script nonce="${nonce}" src="${rulePickerUri}"></script><script nonce="${nonce}" src="${rightViewShellUri}"></script><script nonce="${nonce}" src="${scriptUri}"></script></body></html>`;
}
