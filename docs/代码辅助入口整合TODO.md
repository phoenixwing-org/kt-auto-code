...
2. 点击入口后，Primary 的第三个 Block 显示可整体折叠、带轻边框与独立 Header 的“功能目录”Tree。标题右侧显示总数 `（6）`；当前分组为：`C++ 整理`（3 项：头文件引用修正、成员排序、头文件 ASCII）、`文件工具`（2 项：编码、UUID）和 `CAA`（1 项：CAA UI），每个分组 Header 右侧以紧凑的 `（3）` 样式显示下级功能数。目录内容与 Git Block 一样贴齐当前工具 Block 的左右边界，不另留卡片空隙；当前功能以选中背景和左侧细线标识。拥有 Primary 内部操作区的叶子点击后自动收起“功能目录”，立即露出下方的功能 Block；只打开独立编辑器 View 的“头文件引用修正”不收起目录，避免 Primary 看起来为空。该次入口和当前目录同时写入 KT Auto Code Output。整体目录、分类折叠与内部 Block 折叠保存到 VS Code 用户级 `globalState`，不写工程配置。
...
6. Primary 内部叶子统一使用仓内 `ktcCodeAssistantFeatureBlock` 外壳，拥有相同的 Header、折叠、`×`、操作区和无障碍属性。`×` 只关闭当前叶子、释放其临时结果并返回功能目录，不关闭第三个主 Block。新增内部叶子不得重新使用无 Header 的共享按钮行。
