...
- Side Bar 使用 `CAA 规则`打开统一添加对话框，同时列出“完整名称 I/E”和“仅替换末词段 I/E”。
- 完整名称模式作为常规 CAA 重命名选项排在前面并默认勾选；末词段模式保留 AutoCode 兼容行为，但默认不勾选。
- 两种模式会产生相同 Source、不同 Target，不能同时加入；按对话框顺序保留第一个选中项，已有 Source 不再显示为候选。
- 用户手工编辑过的行仍按自定义规则保留，不静默覆盖。
- 每行按钮复用同一对话框，明确显示模式名称，并隐藏已存在或当前行无法生成的选项。
