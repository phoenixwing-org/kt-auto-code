...
0. **[发布验证：0.7.1]** Auto 自身目标版本为 `0.7.1`。Phoenix Wing 根包 `0.7.1` 不进入 Auto manifest；已精确升级已发布的 `code-core`、`git-core`、`git-node`、`kt-codegen` `0.6.4` 和 `run-core`、`run-node` `0.6.3`，删除本地候选声明。现在在 Registry 模式重跑测试、打包和 VSIX 门禁；不得写入本地路径。
