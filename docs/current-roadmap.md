...
- **0.9.0 收尾**：自动代码、搜索替换、项目改名、Run、编译工具已获用户初步 UI/正式一致性认可；剩余重点为头文件引用修正、Git，以及回归与制品门禁。逐项状态见 [分页面 TODO](0.9.0-Right工具Primary迁移点检.md)，不把外观认可当作完整功能验收。
- **0.9.1 计划**：代码辅助下仅 Primary 工具的内部文字工具栏与 UI Preview；见 [专项计划](0.9.1-代码辅助Primary整改计划.md)。本轮不改这些页面或版本号。
