...
- **0.9.2 明确延期项**：Git“合并区间”Right 原型补齐，按用户新决定移出 0.9.1；见[0.9.2 计划](0.9.2-Git合并区间原型计划.md)。清理快捷栏和缓存清理仍为后续待评审事项，本轮不实现。
- **0.9.2 自动代码文档标签 TODO**：Right 标签由重复的“自动代码”改为 JSON 文件名；用户明确同名文件也不追加目录。只调整 `editorViewController.ts` 创建与更新 Panel title 的来源，保持 URI 去重、一 JSON 一 View、页面内“自动代码 · JSON @ 目录”Header 和工具注册文案。当前是标题相同，不是已证实的重复建页；0.9.1 不实现。实施时同步前端文档标题契约的 JSON 文档标签例外及回归。
