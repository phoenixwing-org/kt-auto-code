...
最新状态：Auto 0.9.3 已修复 0.9.2 的 Primary 初始化、Right 清理消息接收、当前目录范围与长路径布局问题；用户于 2026-09-11 确认本地安装测试通过，并反馈已发布到插件市场。源码与验收记录归档于固定 `worktrees/kt-auto-code-working` 的 `v0.9.3` 分支；AI 未执行市场上传或 Git push，不重新打包覆盖用户已验收包。当前依据见[发布缺陷与 0.9.3 热修复](0.9.2-发布准备.md#093-紧急修复与验收2026-09-11)，旧发布准备记录作为历史保留。

发布决定（2026-09-10）：用户已明确**取消 0.9.0 发布**，不再只是暂停；不继续为该版本打包或上传市场。已有本地归档及历史回执保留，不删除或重写。当前 `codex/v0.9.2` 收口正式 YAML 清理、编译工具原型闭环与 Combo 回填注释修复，压缩归档后同步主库 `user`；**等 Wing 发布完成再升级精确依赖并打包**。范围、验证与待测项见[0.9.2 归档与发布准备](0.9.2-开发计划.md#本轮归档与发布准备2026-09-10)。

## 已完成基线

- Code、CAD 两个扩展可以独立 typecheck、打包为 VSIX，并由制品检查验证必要文件。
- 自动测试覆盖纯核心、宿主 adapter、Codegen 文档模型、事务与回归场景；精确数量由 CI 结果维护，不作为源码断言。
- Auto 不直接依赖 Wing 根聚合包，manifest 与 lockfile 继续精确锁定 Registry 的 `code-core`、`git-core`、`git-node`、`kt-codegen` `0.6.4` 和 `run-core`、`run-node` `0.6.3`；0.9.1 内测使用本地 Wing 根包 `0.7.3`，最终源码与制品来源以[内测收尾](0.9.1-内测收尾.md)为准，不据此宣称 scoped 包已发布。本地路径、override 与 lockfile 回退继续由 `verify:wing-dependencies` 阻止。
...
- **0.9.2 本地候选冻结**：以[开发计划与验收](0.9.2-开发计划.md)为该冻结批次范围；Git Right 内存原型、JSON 文件名标签、构造函数标记规则 `1.0.1` 和 PnwCombo 收尾已实现。该批未开启清理对话框与编译工具后续；正式 Registry 与 Windows/辅助技术验收不由本地通过替代。
- **冻结后的新授权**：用户已重新开启编译工具与清理后续，先按[后续原型计划](编译工具/后续原型计划.md)补编译草稿/清单导入、多 cleanup.yaml 逐文件清理、原生编辑器请求、窗口/任务状态。清理规则最终方案为简单使用，当前规则交给原生未保存 YAML、用户自行保存；不做内置多 tabs/独立文件草稿、规则导入或自建保存对话框。先 Preview、再明确正式接线；新源码与证据不覆盖已冻结 VSIX、版本归档或哈希。
- **0.9.0 发布已取消**：用户明确取消本次 Marketplace 发布，不再继续该版本打包或上传；保留旧归档和未提交准备改动的本地备份，不把已能构建描述为适合发布。
- **0.9.0 本地归档**：头文件、Git、目录同步和三视图关闭修订已纳入内测包；历史冻结记录及 VSIX 保留，不被本轮覆盖。人工未验项与 Wing Registry 前置条件见[发布冻结清单](0.9.0-发布冻结清单.md)。
- **0.9.1 本地内测收尾**：五个仅 Primary 工具的共享文字工具栏、Preview 与正式接线已完成，Git 正式合并图、安全切换、remote 标记及提示交互已修正并本地提交。按用户要求完成版本、测试、内测包与来源归档；见[内测收尾](0.9.1-内测收尾.md)和[专项记录](0.9.1-代码辅助Primary整改计划.md)。不推送或公开发布，不覆盖 0.9.0 制品。
- **Git Right 原型已从 0.9.1 延期后在 0.9.2 实现**：见[专项记录](0.9.2-Git合并区间原型计划.md)。后续清理新授权单列在上项，不追溯加入 Git 专项或冻结包。
- **0.9.2 JSON 标签已实现**：创建及更新均使用文件名，同名文件不追加目录；保持 URI 去重、一 JSON 一 View、内部 Header 和工具注册文案。单元回归及真实 VS Code 同名文件、重复打开、独立标题更新已通过，不回改 0.9.1 冻结。
- 下方 0.8.3 优先级为此前基线记录，不再作为当前收尾版本的推断依据。

历史背景（0.8.3 规划时）：当时的下一发布目标范围见[0.8.3 开发计划](0.8.3开发计划.md)。当时另一台机器的 `0.8.2` 正在修改批量编译、代码辅助和 Tree，约定不追踪中间状态、待发布后整体合入；这不是当前 0.9.1/0.9.2 的分支或发布指令。
...
## 2026-09-10 远程 BUG 记录摘入

来源提交 `7e58f09e1f9b` 仅包含两份 bug 文档和一张截图，本轮选择性复制记录，不迁入其父历史、不改写既有冻结制品。

- [Combo 下拉配色](bug/自动代码-Combo深色配色问题.md)：本地 Wing 源码已有修正，Mac 样式测试 7/7 通过；Windows 实机待测，仍未进入冻结包或 Registry 发布。
- [Combo 回填注释缺失](bug/自动代码-Combo参数回填缺少注释行.md)：已按新增授权修复，生成规则与 Auto 缓存版本同步升级 `1.0.2`；定向回归通过，Windows/CAA 运行验收仍待测，不改写原冻结包。
- [Windows 导出脚本首行错误](bug/编译工具-Windows导出脚本首行错误.md)：待定位，尚缺原文件、完整错误及 PowerShell/调用方式；不将现有源码的 BOM 和版本声明当作问题已解决的证据。

## 2026-07-20 简单 TODO 收口

- **编译工具统一项目表（已落地）**：schema 2 已以单一表格替换旧多行文本框，支持目录录入/探测、启用、Git 快照与组合操作；见[现行说明](编译工具/README.md#schema-2-统一项目表)。后续补的是导入、可交互原型与 provider，而不是重新创建统一表；新范围见[后续原型计划](编译工具/后续原型计划.md)。
