...
Codegen 当前工具 Block 与命令面板应提供“打开 JSON / 导入 CSV / 全部应用 / 刷新列表 / 扫描候选源码 / 复制运行诊断”。这些动作不占用原生 View Header，也不进入 Toolbar Strip 的 Ribbon 定制 `…`。执行“复制运行诊断”后，粘贴内容应以 `kt.codegen.runtime-diagnostics` 开头并显示 5 份文档；诊断不包含表格单元格和源码内容。
...
2. Codegen 当前工具 Block 或命令面板执行“复制运行诊断”后复制的内容。
