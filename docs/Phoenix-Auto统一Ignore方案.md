...
原生 View Header 当前依次提供“目录显隐、忽略范围、设置”三个核心图标。“目录显隐”只控制目录 Row 展示且不丢上下文；“忽略范围”打开独立 `ignoreSettings` 逻辑工具，“设置”打开 `environmentSettings`。后二者复用 Tool Area 内同一个无 Header Tool Surface，不增加一级区域。搜索替换内容内另有紧凑的“忽略范围”折叠区，用于在执行前看到并切换当前三个来源。
