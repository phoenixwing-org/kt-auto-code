...
原生 View Header 只按“Ignore → Settings”提供两个固定全局动作；不再放目录显隐、搜索预览或其他业务快捷入口。Ignore 打开独立 `ignoreSettings` 逻辑工具，Settings 打开 `environmentSettings`；二者都复用第三个一级区域 Current Tool Block，不增加第四个一级区域。搜索替换内容内另有紧凑的“忽略范围”折叠区，用于在执行前看到并切换当前三个来源。

Ignore View 按以下顺序组织：

1. **来源与写入**：切换三个读取来源；用 radio 二选一确定本次编辑目标。默认选择最近 Git 仓库根 `.gitignore`，只有用户主动切换时才使用当前目录 `.phoenix/.ignore`；Git 目标不可用时才自动回退 Phoenix。添加、去除和去重先更新 VS Code 文档草稿，只有用户明确点击“保存”才写盘。
...
│     展开时保留图标与文字，紧凑时只显示同一 Ribbon 图标 │
├─ 目录（固定单行）────────── [当前目录 ▾] [文件夹] ┤
├─ ▾ 当前工具标题 ─────────────────────────────── [×] ┤
│  Current Tool Body（唯一纵向滚动边界）                 │
│  此处显示 Ignore、设置或其他当前逻辑工具            │
└─────────────────────────────────────────────────────┘
```

顺序与职责固定为 Toolbar Strip → 固定单行目录 → Current Tool Block。Current Tool Header 保留折叠按钮、标题与明确的 `×`；只有 Body 可以纵向滚动。底部“打开项”栏如果继续试验，只能嵌在 Current Tool Block 内部，不得成为第四个一级区域。
...
所有添加都先读取当前目标的最新文档草稿，再统一路径分隔符并清理前导 `./`，按精确规则身份检查；已经存在的规则不会重复追加，同一次请求中的重复输入也只添加一次。尾部目录斜杠必须保留，`foo` 与 `foo/` 是两条不同规则。去除只作用于用户明确选择的目标和规则，必须保留注释、无关规则及另一来源内容。空操作不得创建文件。

“修正（N）”只处理 radio 当前选中的目标文档；当前修正项是去除已有重复规则。它以目标文档当前草稿为准，保留第一次出现的位置、原始拼写和顺序，删除后续 exact-identity 重复行，同时保留注释、空行与原换行风格。比较时只采用 Phoenix 当前支持的规范化子集（首尾空白、路径分隔符和前导 `./`）；目录意图与大小写仍属于身份，因此 `foo`、`foo/`、`Foo/` 互不合并。修正不会自动保存，用户可先在编辑器检查，再点击“保存”；保存会写入该目标文件当前的全部未保存编辑，而不只是本次修正结果。
...
- Ignore View 的目标 radio 默认选中最近 Git 根 `.gitignore`，并始终读取打开但未保存的目标文档；只有用户主动选择 Phoenix，后续添加、去除或去重才修改 `.phoenix/.ignore` 草稿。不在 Git 仓库内时允许回退 Phoenix，但必须在界面标明 Git 目标不可用。
- 添加、去除和去重都不隐式保存。目标存在未保存草稿时才启用“保存”；其 Tooltip 必须说明会保存当前目标的全部草稿，避免用户误以为只写入插件刚产生的变更。
...
### Ignore View 人工点检（待完成）

- [ ] 在 Git 目标连续两次添加同一条/同一组规则；第二次不再追加，混合添加时只写入缺少的规则。
- [ ] 在当前草稿手工放入两组重复规则；“修正（N）”显示准确数量，点击后仍保持未保存状态。
- [ ] 修正后首条规则、注释、空行、原顺序与 LF/CRLF 不变；`foo`、`foo/`、`Foo/` 仍是三条独立规则。
- [ ] 点击“保存”后磁盘内容更新、编辑器未保存标记消失、修正计数归零；保存前可以撤销修正。
- [ ] 切换 Git / Phoenix radio 后，修正和保存只作用于当前目标；无重复或目标不存在时修正禁用且不创建文件。
- [ ] 深色、浅色和高对比主题下，目标、禁用原因、修正计数、Tooltip 与键盘焦点均可读。
