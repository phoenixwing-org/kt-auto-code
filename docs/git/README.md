...
- 合并区间必须由当前同一分支上相邻的连续节点组成，不能跨分支或跳过中间 commit。提交图可以显示多个本地分支，但跨分支勾选不再误报为 commit 已变化。打开 View、带入勾选数、预检开始/通过/失败、执行开始/取消/成功/失败都以紧凑日志写入同一个 KT Auto Code Output，并统一使用 `[INFO] / [OK] / [ERROR]` 等级；成功日志包含旧/新 HEAD、区间数量、备份 ref 和未 push，页面保留当前提示，不再额外弹出预检气泡。用户修改勾选会作废仍在运行的旧自动预检，旧结果不能覆盖新的选择。
...
- [ ] 非当前分支连续区间、目标分支锁定、自动 checkout 与 checkbox 旁范围拖动条暂不实现；先按[计划中的安全边界](Git-Primary-Block可行性与实施计划.md#701-待讨论非当前分支区间与范围拖动)继续讨论。当前仍只允许当前检出分支最近线性段上的连续普通 commit。
