...
- `更多 commit` 的操作行位于折叠内容之外，固定显示“下一条 / 下 5 条 / 合并区间”，所以历史收起时仍可操作。前两项在历史末端禁用；“合并区间”始终可进入单例 View，0/1 条仅作为初始选择，至少 2 条时才执行预检。Host 仅接受当前摘要中已知的 OID，Wing 仍重新判定连续区间与安全条件。
