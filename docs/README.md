...
- [0.8.3 发布准备](0.8.3发布准备.md)（等待 0.8.2 整体合入的候选门禁、Windows 点检与制品回执）
- [0.8.3 开发计划](0.8.3开发计划.md)（避开 0.8.2 重叠文件的下一发布目标简表）
...
- [产品功能归属矩阵](产品功能归属矩阵.md)（Auto ↔ Desk 减法矩阵、Desk 退役门禁与 Wing `capabilityId`）
...
- [Phoenix Auto 统一 Ignore 方案](Phoenix-Auto统一Ignore方案.md)（插件内置、Git、自定义三来源，以及 Auto Code/CAD/Wing 边界）
...
- [工作目录、Ignore 与设置 View 改造计划](工作目录与统一设置View改造计划.md)（固定单行目录、原生 Header 双入口与 Ignore/设置独立工具）
...
- [项目改名第二阶段：结构化规则与正则可行性](项目改名第二阶段-结构化规则与正则可行性.md)（受控词段、通用前缀、遗漏探测、后置正则与 Go/No-Go 门禁）
