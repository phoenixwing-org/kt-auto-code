...
.page{max-width:1000px;margin:0 auto;padding:20px 24px 36px}.head{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;border-bottom:1px solid var(--vscode-panel-border);padding-bottom:13px}
h1{font-size:20px;font-weight:600;margin:0 0 5px}.muted{color:var(--vscode-descriptionForeground)}.status{margin:13px 0;padding:7px 9px;border-left:3px solid var(--vscode-focusBorder);background:var(--vscode-textBlockQuote-background)}.status.error{border-color:var(--vscode-errorForeground);color:var(--vscode-errorForeground)}
.toolbar{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:14px 0}.groups{border-top:1px solid var(--vscode-panel-border)}.group{padding:12px 4px;border-bottom:1px solid var(--vscode-panel-border)}
...
@media(max-width:620px){.page{padding:14px}.head{flex-direction:column;gap:8px}.group-head{grid-template-columns:22px minmax(0,1fr)}.badges{grid-column:2;justify-content:flex-start}}
</style></head><body><main class="page"><header class="head"><div><h1>Ignore 分析推荐</h1><div class="muted" id="workspace"></div></div><div class="muted">逐组确认，不自动保存</div></header>
