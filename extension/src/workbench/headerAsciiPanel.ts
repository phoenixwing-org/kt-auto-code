...
type PanelMessage = { type: "ready" } | { type: "openIssue"; file: string; line: number };
...
export class HeaderAsciiPanel {
  ...
  private constructor(private readonly panel: vscode.WebviewPanel) {
    ...
    panel.webview.onDidReceiveMessage((message: PanelMessage) => { void this.onMessage(message); });
  }
  ...
  private async onMessage(message: PanelMessage): Promise<void> {
    ...
    if (!root || !isInsideRoot(root, message.file)) return;
...
*{box-sizing:border-box}body{margin:0;background:var(--vscode-editor-background);color:var(--vscode-foreground);font:13px var(--vscode-font-family)}
