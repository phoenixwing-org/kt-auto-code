...
export class HeaderAsciiPanel {
  ...
  private async onMessage(message: KtcHeaderAsciiPanelMessage): Promise<void> {
    ...
    if (message.type === "run") {
      await vscode.commands.executeCommand(`ktAutoCode.headerAscii.${message.action}`);
      return;
    }
...
.page{width:100%;max-width:none;margin:0;padding:12px 16px 24px}h1{font-size:18px;font-weight:600;line-height:1.25;margin:0;white-space:nowrap}.muted{color:var(--vscode-descriptionForeground)}
.head{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:0 0 9px;border-bottom:1px solid var(--vscode-panel-border)}.title-line{display:flex;align-items:baseline;gap:10px;min-width:0}.head .muted{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.actions{display:flex;gap:6px;flex:0 0 auto}button{min-height:26px;padding:3px 9px;border:1px solid var(--vscode-button-border,transparent);border-radius:2px;color:var(--vscode-button-foreground);background:var(--vscode-button-background);cursor:pointer}button.secondary{color:var(--vscode-button-secondaryForeground);background:var(--vscode-button-secondaryBackground)}button:disabled{opacity:.5;cursor:default}.status{margin:10px 0 8px;padding:6px 9px;border-left:3px solid var(--vscode-focusBorder);background:var(--vscode-textBlockQuote-background)}.status.error{border-left-color:var(--vscode-errorForeground);color:var(--vscode-errorForeground)}
.summary{display:flex;gap:5px;margin-bottom:10px}.stat{min-width:0;padding:5px 8px;background:var(--vscode-editorWidget-background);border:1px solid var(--vscode-panel-border);border-radius:3px}.stat strong{font-size:15px}.stat span{margin-left:5px;font-size:11px;color:var(--vscode-descriptionForeground)}
...
</style></head><body><main class="page"><header class="head"><div class="title-line"><h1>头文件 ASCII 修正</h1><span class="muted">非 ASCII 字节预检结果</span></div><div class="actions"><button id="scan" class="secondary" title="按当前范围重新预检">重新预检</button><button id="fix" disabled>修复</button></div></header><div id="status" class="status">等待预检…</div><div id="summary" class="summary"></div>
...
$("scan").onclick=()=>vscode.postMessage({type:"run",action:"scan"});$("fix").onclick=()=>vscode.postMessage({type:"run",action:"fix"});window.addEventListener("message",e=>{const s=e.data.state;if(!s)return;const running=s.status==="running",issues=s.issueFiles??0;$("scan").disabled=running;$("fix").disabled=running||issues===0;$("status").className=s.status==="error"?"status error":"status";$("status").textContent=s.message||"";const items=s.results||[];$("summary").innerHTML=running?"":'<div class="stat"><strong>'+(s.scanned??0)+'</strong><span>已扫描文件</span></div><div class="stat"><strong>'+issues+'</strong><span>问题文件</span></div>';$("rows").innerHTML="";$("empty").style.display=items.length?"none":"block";if(!items.length&&s.status==="done")$("empty").textContent=issues===0?"未发现非 ASCII 问题":"没有可显示的结果";for(const item of items){const tr=document.createElement("tr");const issues=item.issues||[];let detail=issues.slice(0,3).map(x=>"L"+x.line+":C"+x.column+" "+x.fromLabel+"→"+x.toLabel).join(" · ");if(issues.length>3)detail+=" · ……等 "+issues.length+" 处";tr.innerHTML='<td class="path" role="button" tabindex="0">'+esc(item.relativePath||item.file)+'</td><td class="count">'+item.issueCount+'</td><td class="detail">'+esc(detail)+'</td>';const pathCell=tr.querySelector(".path"),open=()=>vscode.postMessage({type:"openIssue",file:item.fullPath,line:item.topLine});pathCell.onclick=open;pathCell.onkeydown=x=>{if(x.key==="Enter"||x.key===" "){x.preventDefault();open()}};$("rows").appendChild(tr)}});vscode.postMessage({type:"ready"});</script></body></html>`;
}
