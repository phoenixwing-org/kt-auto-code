...
describe("associated rule picker architecture", () => {
  ...
  it("Panel、构建、架构门禁、Browser 夹具与 VSIX 制品门禁全部接线", () => {
    ...
    const fixture = source("../../test-fixtures/associated-rule-picker.html");
    ...
    expect(build).toContain("esbuild.build(associatedRulePickerOptions)");
    expect(build).toContain("associatedRulePickerContext.watch()");
    expect(architecture.viewRoots).toEqual(expect.arrayContaining([
      "extension/src/sidebar/associatedRulePicker.ts",
      "extension/src/sidebar/associatedRulePickerEntry.ts",
    ]));
    ...
    expect(vscodeIgnore).toContain("test-fixtures/**");
  });
});
