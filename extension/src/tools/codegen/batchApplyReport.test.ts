...
describe("Codegen 全部应用轻量报告", () => {
  it("从结构化结果汇总状态与错误，不解析 Output", () => {
    const items = [
      {
        documentId: "file:///workspace/A.json", fileName: "A.json", displayPath: "file:///workspace/A.json", health: "success" as const,
      ...
      {
        documentId: "file:///workspace/B.json", fileName: "B.json", displayPath: "file:///workspace/B.json", health: "error" as const,
        ...
        issues: [{ severity: "error" as const, code: "marker.missing-end", message: "未闭合", path: "/src/B.cpp", line: 9 }],
      },
    ];
  ...
  it("收集 error/warning，行号转成用户可见 1-based 并去重", () => {
    ...
    expect(ktcCodegenBatchApplyReportIssues(diagnostics, "/A.json")).toEqual([
      { severity: "error", code: "marker.missing-end", message: "未闭合", path: "/a.cpp", line: 9 },
      { severity: "warning", code: "notice", message: "提示", path: "/A.json" },
    ]);
  });
});
