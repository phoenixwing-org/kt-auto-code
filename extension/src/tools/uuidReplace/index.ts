...
export const uuidReplaceTool: KtTool = {
  ...
  registerCommands(context): void {
    ...
    context.subscriptions.push(
      ...
      vscode.commands.registerCommand("ktAutoCode.uuidReplace.addToWorkset", () => { void addUuidResultsToWorkset(); }),
    );
  },
  async handleMessage(message: WebviewInboundMessage, ctx: ToolRunContext): Promise<void> {
    if (message.type === "run" && message.toolId === this.id) {
      if (message.action === "addToWorkset") await addUuidResultsToWorkset();
      else await runUuidAction(
  ...
  async runAction(action: string, ctx: ToolRunContext): Promise<void> {
    if (action === "addToWorkset") await addUuidResultsToWorkset();
    else await runUuidAction(action === "fix" ? "apply" : "scan", ctx);
  },
};
...
async function addUuidResultsToWorkset(): Promise<void> {
  const root = vscode.workspace.workspaceFolders?.[0]?.uri;
  if (!root || session?.root !== root.fsPath || !session.files.length) { void vscode.window.showInformationMessage("请先在当前工作区生成 UUID 扫描结果。"); return; }
  try { await ktcAddResultFilesToWorkset(root, session.files.map((file) => file.relativePath), "UUID 替换"); }
